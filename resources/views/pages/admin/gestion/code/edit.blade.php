...
@section('content')
    <div class="container  py-12 ">
        ...
        <section class="mt-8">
            ...
            <form action="{{ route('admin.code.update', $data->code) }}" method="post">
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="plan" id="plan"
                            ...
                            <option value="expert" @if ($data->plan == 'expert') selected="selected" @endif>Expert
                            </option>
                            <option value="pro" @if ($data->plan == 'pro') selected="selected" @endif>Pro
                            </option>
                            <option value="basic" @if ($data->plan == 'basic') selected="selected" @endif>Basic
                            </option>
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="code" id="code"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            cols="30" rows="10" required>{{ $data->code }}</textarea>
                        <x-input-error :messages="$errors->get('code')" class="mt-2 " />


                    </div>
                    <div>
                        <label for="is_validated" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">État
                            de validité</label>
                        <select name="is_validated" id="is_validated"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                            required>

                            <option value="0" @if ($data->is_validated == 0) selected="selected" @endif>invalide
                            </option>
                            <option value="1" @if ($data->is_validated == 1) selected="selected" @endif>valide
                            </option>
                        </select>
                        <x-input-error :messages="$errors->get('is_selected')" class="mt-2 " />
